...
@Component({
  selector: 'app-mobile-transfer-list',
  standalone: true,
  ...
  styleUrl: './mobile-transfer-list.component.css'
})
export class MobileTransferListComponent implements OnInit, AfterViewInit {
  mobileList: any[] = [];
  portalList: any[] = [];
  ...
  GetmobileTransfers(): void {
    ...
    this.mobileTransferService.GetmobileTransfers(filters).subscribe({
      ...
      error: (err: any) => {
        console.error('Error fetching mobile transfers:', err);
      },
    });
  }
  ...
  editTransfer(mobile: any): void {
    this.router.navigate([`/admin/mobile-transfer/editmoney/${mobile.TransferID}`], {
      state: { mobileData: mobile } // Pass portal data using state
    });
  }
  ...
  excelDownload(title: string): void {
    ...
    let dataToExport = this.mobileList.map((x: any) => ({
      ID: x.TransferID,
